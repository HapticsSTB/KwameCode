...
def create_predfilename(name, task_str, data_dir):
    test_path = data_dir + '/' + name + "_task" + task_str + '_' + time.strftime('%m-%d_%H-%M')
...
while True: # Runs once if args.pedal is false
    try:
        ...
        if args.write or args.video:
            test_filename = create_predfilename("tempcsv", args.task, 'tempcsv')
...
#automatically calls predictor on the matlab end
ratings = np.array(eng.STB_pred(int(args.task), 1))
print "ratings: "
print ratings
