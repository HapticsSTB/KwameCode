classdef STBData < handle
	...
	methods 
        ...
        function obj = STBData(dataDir, csvFileName, varargin)
            ...
            if (ismember(str2double(fileName.name(13)), inputs.task))
                disp(['Loading: ' fileName.name])
                % Load rawData, rating(not recorded yet) and
                % youtube address from .mat
                load([dataDir '/' fileName.name], 'rawData', 'youtube', 'youtube_short', 'score', 'rater');
                ...
                obj.time = datetime(fileName.name(15:end-4), 'InputFormat', 'MM-dd_HH-mm');
